...
| Ideal Producer with Categories | 300 pcs per hour | APA: Value from event `Pieces_Produced` count<br />APT: Value from event: `Availability` = String `up`, `Planned maintenance`, `Manual stop`<br />AQA: Value from event `Pieces_Ok` count | 80, 80, 100, 100 | |
